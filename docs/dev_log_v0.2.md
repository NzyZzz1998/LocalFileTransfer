...
- 用户确认最后一个本机管理页关闭后安全退出，远端未保存文件优先保留。增加独立 `/local-manager` WebSocket，仅实际 loopback 请求且 Origin 精确匹配才能登记；页面角色不决定主机权限。只有最后一个已登记连接断开才启动 5 秒宽限，从未打开管理页不退出。
- 复用关闭协调器；自动退出意图由本机连接创建，远端 `shutdown_retry` 只能继续匹配 requestId 且曾阻塞该次自动退出的请求。管理页重开取消自动意图，手动关闭优先；保存早于其他页面清理确认时记住重试，不轮询等待用户操作。
- 浏览器 beforeunload 仅请求原生离开确认，pagehide 才开始退出清理，避免点“留下”也破坏传输。管理连接覆盖首页，角色切换不重建；断线只做有界重连，不新增常驻计时器。
- 保存/放弃后保留控制连接到下一次协调清理；取消自动退出不能丢弃未完成或失败的清理所有权。异步取消带请求代际核对；新关闭请求复用已有清理工作但接管新的 requestId、模式与取消状态。
- TDD 反证、两项独立复核发现、源码和 Windows 原生产物验证及当前校验值见 progress。没有新增依赖或辅助进程，不自动提交/推送/发布。

### 2026-09-14 / 关页功能授权推送

- 用户明确要求先推送本次关页更新，计划今天 Mac 浏览器连接 Windows 验收。沿用 test 分支，仅提交本次实现、测试和交付说明，不合并 main、不创建 PR/tag/Release。
- 推送前核对远端无分歧，重新跑完整 Bun 测试、Windows 产物 smoke 和原生首页关页退出；更新授权状态与双机验收入口。Git 推送不包含被忽略的 dist，远端构建结果必须按本次提交另行核对。

### 2026-09-14 / VPN 验收补修：主动中转与首次失败诊断

- 用户反馈 Windows↔Mac 双向互传通过；纠正先前选项后明确：双方旧巷 VPN 在传输前已开启，文件开始传输后失败，无截图或诊断。未据此判定 ICE、VPN 规则或文件处理是根因，也未改动 VPN、路由或防火墙。
- 授权范围为传输前主动申请本地中转、保留失败诊断，仍属于 v0.2 补修。本轮不升版本、不新增依赖、不写持久诊断日志，不提交/推送/tag/Release。
- 使用 oil-frontend 信息/动作与状态规则，将已有中转按钮从失败面板提到配对后的共享操作区；直连连接中或已验证、但尚未接受文件时均可申请。沿用双方中转确认及接收文件确认。待确认/中转就绪/传输中收起改路入口，处理器仍验证状态，不以隐藏按钮替代约束。
- 申请时退役未开始传输的旧引擎、存储预检和 RTC，不终止配对房间；中转被拒后可重新申请，直连重试仍重新建房并保留文件清单。旧回归发现对方离开后误藏重试按钮，已将新建房间重试与需要活跃房间的中转动作分开。
- 诊断按当前会话记录角色、尝试路线、接收确认字节和整批总字节，更新不依赖 250 ms 的进度绘制节流。首次终态冻结完整快照，后续清理错误不覆盖；显式改路开启新阶段。全程白名单，不含文件名、路径、接收码、IP、SDP、凭据或任意关闭原因。
- 测试先红：真实直连就绪/协商中时中转入口隐藏；真实 262144 字节已接收并 ACK 后断开 `/ws`，旧诊断被覆盖为 `TRANSFER_FAILED`/`failed`。新用例不替换业务模块，仅在原生 offer、文件读取或 socket 边界施加故障；下载使用真实加密传输与字节比对。独立复核继续覆盖确认文件前断开配对连接的首次原因。
- 最终测试结果、Windows 产物身份及真机待验收范围以 progress 的最新记录为准。

### 2026-09-14 / 主动中转与诊断补修授权推送

- 用户在本地修复交付后明确要求“推送”；沿用 test，只提交本轮实现、测试和更新后的说明，不合并 main、不创建 PR/tag/Release。
- 推送前 fetch 确认远端 test 无分歧，重新验证 Bun 测试、主动中转浏览器组和 Windows 原生产物 smoke；不修改 VPN 或重新常驻启动服务。Git 不包含 dist，CI 必须按本次提交另行核对。
