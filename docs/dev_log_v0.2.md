...
- 授权：用户要求关闭相关服务不留遗留，并以“go”确认补修。本轮仅本地修改、验证与重建产物，不自动提交、推送或发布。
- 原因与反证：Windows 二进制虽然退出 0、端口释放，但已传完的真实 RTC 仍为 connected、DataChannel 为 open，页面保留 1 秒倒计时。开发 `bun --watch` 还留下监听父子进程；两类问题均先实测后修复。
- 服务端：受保护的本机 POST 向现有信令连接发送带 requestId 的关闭通知；收齐清理确认才停止监听。未保存、删除失败或 10 秒内未回应明确阻止关闭；并发请求共用一轮确认，期间拒绝新连接。仅 CLI 完成关闭时退出进程，使 watch 父子进程一并退出，不让导入测试服务触发 process.exit。
- 浏览器：停止 RTC/中转、心跳与页面计时器，解绑引擎监听，等待正在进行的读取、存储预检、创建、写入及 finalize，等待浏览器下载交接后释放 Blob/URL/文件闭包。收到 HTTP 成功后仍检查服务不再可访问才显示关闭完成。
- 文件保护：完整批次及部分已收完文件均受未保存保护。发送方 cancel、RTC close 和关闭通知先后顺序不同也保留已收完文件的保存入口；保存或明确放弃后重试。原文件、已下载文件与其他页面存储不删除。
- 清理失败：OPFS 只登记本模块创建的精确临时文件，不扫描其他标签页或历史文件。清理失败保留登记并有界重试；取消预检不会遮蔽真实删除失败。正常返回首页也要等清理完成再断信令，失败页面继续参与关闭确认，不能从参与者列表消失后误报成功。
- 竞态补漏：清理与关闭重叠时先 ACK 再离开；成功取消预检不能被误判为清理失败从而阻止下一轮；页面销毁会取消计时器，因此关页时立即触发下载临时文件清理，不等待已经销毁页面的 5 秒回调。
- 回归：新增真实 HTTP/WS/watch、引擎异步 disposal、OPFS 清理测试。新增 `tests/e2e_shutdown_test.py`，统一 runner 增加 shutdown 组；真实原生 RTC/WS、下载字节、临时文件、计时器、PID 与端口均作为断言。强化的部分批次测试故意延迟接收方关闭通知，已先观察 cancel 先到的失败再修复。
- 最新验证数值和候选身份见 `docs/progress_v0.2.md`；Windows 浏览器使用已安装 Chrome。配套 Playwright Chromium 未安装，本轮不冒称其通过；Mac/Linux 真机与跨设备/TUN 仍需外部验收。

### 2026-09-13 / 公开仓库与功能优先首页

- 用户要求公开项目、优化奇怪广告文案，并进一步明确“不需要标语，直接展示功能”。已完成仓库可见性切换和匿名访问核验；不合并 main、不推送未提交工作树、不创建 Release。
- 删除首页 hero、宣传徽章、航标编号及重复操作词，将发送/接收入口放在第一屏，LAN 地址与关闭入口置于其后。沿用既有配色和组件，删除被移除内容的样式，而非增加新的宣传区。
- 统一静态及动态文案，消除货单/靠岸/摆渡等隐喻；等待、传输、接收、保存使用实际任务名称。保留容量限制、同意中转、下载与未保存风险，明确“无文件留存”仅指服务端。移除无导航行为的品牌链接，演示保存明确标记不下载。
- 首屏可操作性测试先观察旧页面失败，再验证桌面与窄屏通过；既有 UI、真实直连、诊断回归通过，239 项单元/集成测试通过。重建 Windows 程序并核对嵌入资源与正常关闭；最新哈希及验证边界见 progress。

### 2026-09-14 / 授权推送与异机启动说明

- 用户明确要求推送本地更新，并询问在其他电脑拉取后运行；交付目标沿用 `test`，不扩展到 main 合并、PR 或 Release。
- 推送前 fetch 确认本地/远端 test 无分歧，重新执行完整单元/集成测试；README 补全 Git+Bun、克隆 test、安装锁定依赖和首次启动命令，明确只运行一个局域网服务实例。
- 新远端 CI 必须核对实际新提交，不能把此前失败/成功结果当成本轮证据；Mac/Linux 和跨设备/TUN 验收边界保持不变。
