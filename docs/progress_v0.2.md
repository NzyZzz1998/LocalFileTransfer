...
- 当前状态：主动中转入口和首次失败诊断已本地完成，完整回归与 Windows 原生产物验证通过；用户已明确授权本次提交/推送 test。用户已反馈 Windows↔Mac 双向互传通过；双方提前开启旧巷 VPN 时传输中失败，根因及本次补修的双机效果仍待确认。
- 目标版本：v0.2
- 上游来源：`docs/prd_v0.2.md`、`docs/dev_plan_v0.2.md`
- 下游承接：本次授权提交/推送 test → 对应 CI 与同一文件直连/本地中转对照验收；关页完全退出、VPN 场景与吞吐仍待真机补证。
- 当前事实源：本文
- 最后更新：2026-09-14

## 主动中转与失败诊断补修（2026-09-14）

- 用户事实：Windows↔Mac 双向互传已反馈通过；双方提前开启旧巷 VPN 时文件开始传输后失败，实际路线与原因未确认。关页完整退出未获用户真机确认，不将先前观察到无进程等同于验收。
- 交付：允许连接后、接收方接受文件前，发送方可直接点击“改用本地中转”，无需等直连超时。接收方仍须同意路线和文件清单；拒绝可重新申请，对方离开仍可用原文件重新建房。传输开始后不换路、不续传、不自动切换。
- 诊断：当前页面保留首次失败的错误码、阶段、连接状态、角色、尝试路线、已确认接收字节及总字节，清理不覆盖。接收前和传输中断开配对连接均覆盖；返回首页/刷新/关闭会清除内存记录。不含文件名、路径、IP、接收码、SDP、token 或文件内容，不新增后台日志、依赖或服务。
- 已验证：最新 `bun test` 260 pass / 0 fail / 1000 assertions；`python tests/run_browser_tests.py --suite all` 八组完整门禁通过、退出 0，包括手动关闭 10 场景、关页自动退出 13 场景、主动中转 5 场景。诊断组在补齐接收前断线后另行完整重跑，20/20 通过。TDD 红绿证据与独立复核修正见 dev log。
- Windows：重新构建 `dist/dukou-windows-x64.exe`，smoke 核对 10 项嵌入资源，未授权关闭 403、正常关闭 200、进程自然退出 0，无强制清理。原生程序真实主动中转下载字节一致，随后正常退出且端口释放；原生 `home-close` 也通过。
- 当前 SHA256：`DE082A09B26A2CA4048B7E8835E1FAEFF5BEF040C015FF583A3773C909431D29`，与 `dist/SHA256SUMS.txt` 一致；下方旧批次哈希不再代表当前产物。
- 授权推送：用户在本地交付后明确要求“推送”。推送前 fetch 确认 HEAD 与 origin/test 同为 `9ebd30e`，无分歧；重新执行 Bun 测试、主动中转浏览器组及 Windows 产物 smoke。由 test 上包含本记录的提交承载本次补修，最终提交身份与 CI 以 Git/Actions 为准，不沿用旧提交结果。
- 边界：Windows + 已安装 Chrome 本机自动化，不冒充双物理设备 VPN 验收或吞吐提升。不合并 main、不创建 PR/tag/Release，版本仍为 `0.2.0`。Git 不上传被忽略的 dist/EXE；测试仅管理自有进程和独立端口，不操作用户 3000 服务、VPN 或防火墙。
...
- 用户方便时，用同一文件对照默认直连与主动本地中转：允许连接后、确认接收文件前，由发送方点“改用本地中转”，接收方同意路线后再接收。双方 VPN 保持测试前状态；失败时在当前页面复制诊断即可。
- Windows↔Mac 双向互传已获用户通过反馈；本次新补修在双机上的 VPN 对照、关页后完整退出与吞吐仍待确认。Linux 缺少设备不扩展声明。
- 本地补修基线 `test@9ebd30e`；本批已获明确提交/推送 test 授权。核对本批对应 CI，不合并 main、不创建 tag/Release；此前批次记录仅作为历史。
...
- 发布面与最小待执行动作见 `docs/release_checklist_v0.2.md`。
