...
包括直连的中转开启/关闭配置、桌面/窄屏 UI、会话隔离、中转故障、存储异常、地址/诊断和完整关闭回归。可用 `python tests/run_browser_tests.py --suite shutdown` 单独验证手动关闭，或 `python tests/run_browser_tests.py --suite tab-exit` 验证关页自动退出；每个关闭场景自建随机端口的服务，并检查进程正常退出与端口释放。`--browser chromium` 强制使用 Playwright 配套浏览器。
